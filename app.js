...
let lastTapTime=0, lastTapType=null, lastTapI=null, lastTapIdx=null;

const $=id=>document.getElementById(id);

function card(s,r,up=false){
  return {
...
function canStack(a,b){
  return RED.has(a.s)!==RED.has(b.s) &&
         rankVal(a.r)===rankVal(b.r)-1;
}

function canFoundation(c,f){
  return f.length===0
    ? c.r==="A"
    : f[f.length-1].s===c.s &&
      rankVal(c.r)===rankVal(f[f.length-1].r)+1;
}
...
function moveScore(n){
  if(settings().scoreMode!=="none"){
    state.score+=n;
  }
}
...
function settings(){
  return JSON.parse(
    ...
    '{"hints":true,"auto":true,"sound":false,"scoreMode":"standard","theme":"green"}'
  );
}

function saveSettings(o){
  localStorage.setItem("solitaire-settings",JSON.stringify(o));
}
...
function touchPile(type,i,e){
  ...
  if(type==="t"){
    ...
    let idx=Math.floor(
      y/Math.max(35,Math.min(95,rect.width*1.38))
    );

    if(idx>=pile.length) idx=pile.length-1;

    if(idx>=0 && pile[idx].up){
      handleSelection(type,i,idx);
    }else if(pile.length && pile[pile.length-1].up){
      handleSelection(type,i,pile.length-1);
    }
  }else if(pile.length){
    handleSelection(type,i,pile.length-1);
  }
...
function handleSelection(type,i,idx){
  ...
  if(!selected){
    if(type==="t"){
      ...
      // Only a valid descending alternating-colour run
      // can be selected.
      for(let k=idx+1;k<p.length;k++){
        if(!canStack(p[k],p[k-1])) return;
      }
    }
  ...
  if(tryMove(selected,type,i,idx)){
    selected=null;
    render();
    checkWin();
  }else{
...
function takeSelected(){
  ...
  const src=
    selected.type==="t"
      ? state.tableau[selected.i]
      : selected.type==="w"
        ? state.waste
        : state.found[selected.i];

  if(selected.type==="t"){
    return src.splice(selected.idx);
  }
...
function tryMove(sel,to,ti,idx){
  ...
  if(to==="t"){
    ...
    if(
      dest.length===0
        ? first.r==="K"
        : canStack(first,dest[dest.length-1])
    ){
      snapshot();
  ...
  if(
    to==="f" &&
    run.length===1 &&
    canFoundation(first,state.found[ti])
  ){
    snapshot();
...
/*
   Double-tapping a card will first try to move it
   to its correct foundation.

   If that isn't possible, it will try to move the
   card onto a legal tableau pile.

   Only the top card of a tableau pile is eligible
   for automatic movement. This prevents accidentally
   splitting a stack.
*/
...
function doubleTapMove(type,i,idx){
  ...
  const c=src[cardIndex];

  if(!c || !c.up)return;

  /* A double-tapped tableau card must be the
     exposed/top card. */
  if(type==="t" && idx!==src.length-1){
    return;
  }
  ...
  /* If foundation isn't possible, try tableau */
  for(let t=0;t<7;t++){
    ...
    /* Card onto another tableau pile */
    if(
      dest.length &&
      canStack(c,dest[dest.length-1])
    ){
      snapshot();
...
/*
   Detect a double tap reliably on iPhone as well as
   desktop browsers.
*/
function handleCardTap(type,i,idx){
  const now=Date.now();
  ...
  if(sameCard && now-lastTapTime<400){
    lastTapTime=0;
    doubleTapMove(type,i,idx);
    return true;
  }
...
function stockTap(){
  if(state.stock.length){
    ...
    // The card becomes face-up when drawn.
    c.up=true;
  ...
  if(state.waste.length){
    ...
    // Cards are face-down again when returned to stock.
    ...
    if(c){
      for(let f=0;f<4;f++){
        if(canFoundation(c,state.found[f])){
          ...
          state.found[f].push(state.waste.pop());
...
function hint(){
  ...
  for(let i=0;i<7;i++){
    ...
    if(c && c.up){
      for(let f=0;f<4;f++){
        if(canFoundation(c,state.found[f])){
          return flash(
            "Try moving "+c.r+c.s+" to a foundation."
          );
        }
      }
    }
  ...
  if(w){
    for(let i=0;i<7;i++){
      ...
      if(
        ...
        return flash(
          "Try moving "+w.r+w.s+" onto the tableau."
        );
      }
    }
  }
  ...
  for(let a=0;a<7;a++){
    for(let b=0;b<7;b++){
      if(a!==b){
        let pa=state.tableau[a];
        let pb=state.tableau[b];
        let c=pa.at(-1);
...
function render(){
  ...
  stock.onclick=stockTap;

  if(state.stock.length){
    stock.appendChild(cardEl({up:false}));
  }else{
  ...
  waste.className="pile";

  waste.onclick=()=>{
    if(state.waste.length){
      const idx=state.waste.length-1;

      if(!handleCardTap("w",0,idx)){
        handleSelection("w",0,idx);
      }
    }
  };
  ...
  if(state.waste.length){
    ...
    /*
       Double tap is also attached directly to the
       visible card for desktop browsers.
    */
    wc.ondblclick=(ev)=>{
      ev.stopPropagation();
      lastTapTime=0;
  ...
  /* Spacer piles used by the existing layout */
  for(let i=0;i<5;i++){
    const p=document.createElement("div");

    p.className="pile";
    p.dataset.i=i+2;
    p.style.visibility=i<1?"hidden":"visible";
  ...
  for(let i=0;i<4;i++){
    ...
    /*
       A selected card can be placed into the
       foundation by tapping the foundation.
    */
    p.onclick=()=>{
      if(
        selected &&
        tryMove(selected,"f",i,0)
      ){
        selected=null;
        render();
        checkWin();
    ...
    if(f.length){
      ...
      s.innerHTML=
        `<div style="text-align:center;padding-top:20%;opacity:.35;font-size:24px">
          ${SUITS[i]}
        </div>`;
  ...
  for(let i=0;i<7;i++){
    ...
    /*
       Allows a King to be moved to an empty tableau
       by selecting the King and tapping the empty space.
    */
    col.onclick=()=>{
      if(selected){
        handleSelection("t",i,0);
      }
    };
    ...
    state.tableau[i].forEach((c,j)=>{
      ...
      /*
         Tighter spacing on iPhone so the whole game
         fits much better vertically.
      */
      e.style.top=
        (j*
          (
            window.innerWidth<700
              ? Math.min(38,window.innerWidth*.09)
              : 55
          )
        )+"px";
      ...
      e.style.height=
        (
          window.innerWidth<700
            ? Math.min(82,window.innerWidth*.21)
            : 105
        )+"px";
      ...
      e.onclick=(ev)=>{
        ...
        if(!handleCardTap("t",i,j)){
          handleSelection("t",i,j);
        }
      };
...
$("settingsBtn").onclick=()=>{
  ...
  $("hintToggle").checked=s.hints;
  $("autoToggle").checked=s.auto;
  $("soundToggle").checked=s.sound;
  $("scoreMode").value=s.scoreMode;
  $("theme").value=s.theme;
};
